...
const ProfessionalHeader = React.memo(() => {
  ...
  // Ref for the header name to animate with GSAP
  ...
  // GSAP animation for the header name with a cursive writing effect
  useEffect(() => {
    if (headerNameRef.current) {
      // Split text into spans for individual letter animation
      headerNameRef.current.innerHTML = headerNameRef.current.textContent
        .split("")
        .map((letter) => `<span>${letter}</span>`)
        .join("");
      gsap.fromTo(
        headerNameRef.current.querySelectorAll("span"),
        { opacity: 0, x: -20 },
        {
          opacity: 1,
          x: 0,
  ...
  return (
    <>
      <header
        ...
        style={{ height: "56px" }}
      >
        {/* Updated brand name with a designer cursive font style */}
        <Link
          to="/"
          ref={headerNameRef}
